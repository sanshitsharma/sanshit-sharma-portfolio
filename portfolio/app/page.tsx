...
const resumeData = {
  basics: {
    ...
    phone: '408-250-5749',
    links: ['linkedin.com/in/sanshitsharma'],
    summary:
      "GenAI software engineer with 12+ years building distributed systems at AWS, Cisco, and Qualcomm. Currently building Amazon Bedrock's core infrastructure; Model Migration, Model Evaluation and ML pipelines serving thousands of enterprise customers. Combines deep systems expertise with AI-assisted development practices to accelerate delivery velocity while maintaining production-grade quality. Shipped multiple AWS re:Invent launches (2023, 2025). Patent holder.",
  },
  impactMetrics: [
    {
      metric: '80%',
      context:
        'Reduced MTTR via AI root cause analysis using few-shot learning',
    },
    {
      metric: '77%',
      context:
        'Container image reduction (3GB → 700MB) for production pipelines',
    },
    {
      metric: '4w→2d',
      context: 'Reduced model onboarding time from 4 weeks to 2 days',
    },
  ],
  experience: [
    {
  ...
  skills: [
    ...
    },
    {
      ...
      items: ['Kafka', 'gRPC', 'REST APIs', 'PostgreSQL'],
    },
    {
      group: 'Development',
      items: [
        'AI-assisted development (Kiro, Claude Code, Wasabi)',
        'Rapid prototyping',
  ...
  patents: [
    {
      ...
      details:
        'US Patent Application US20200358648A1, Cisco Technology Inc., Filed October 2019',
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            <p className="mt-4 text-sm tracking-widest text-slate-400 uppercase">
              Initializing OS...
            </p>
          </motion.div>
        )}
        ...
        <section className="relative flex flex-col items-start pt-12 md:pt-24 min-h-[70vh] justify-center">
          ...
          >
            ...
            <div className="flex flex-col gap-4 mt-10 sm:flex-row">
              <a
                href="#experience"
                className="inline-flex items-center justify-center px-6 py-3 text-sm font-medium text-black transition-all rounded-lg bg-cyan-400 hover:bg-cyan-300 hover:scale-105 active:scale-95"
              >
                View Experience
        ...
        <section className="relative z-10">
          ...
          >
            {resumeData.impactMetrics.map((item, i) => (
              <div
                key={i}
                className="p-6 transition-all rounded-xl glass-card hover:border-cyan-400/50 hover:shadow-[0_0_30px_rgba(103,232,249,0.15)] group"
              >
        ...
        >
          ...
          <div className="space-y-4">
            {resumeData.experience.map((job, idx) => (
              <div
                key={idx}
                className="overflow-hidden transition-all border rounded-xl glass-card"
              >
                <button
                  onClick={() =>
                ...
                >
                  ...
                  <div className="flex items-center gap-4 mt-2 sm:mt-0">
                    ...
                    <motion.div
                      animate={{ rotate: openExpIndex === idx ? 180 : 0 }}
                    >
                    ...
                    >
                      <ul className="p-6 space-y-3">
                        {job.bullets.map((bullet, bIdx) => {
                          const isHighlight =
                            bullet.includes('re:Invent') ||
                            bullet.includes('Reduced MTTR') ||
                            bullet.includes('Patent granted');
                          return (
              ...
              <div key={i} className="p-6 border rounded-xl glass-card">
                ...
                <div className="flex flex-wrap gap-2">
                  {skillGroup.items.map((skill, j) => (
                    <span
                      key={j}
                      className="px-3 py-1 text-xs border rounded-md text-slate-300 border-white/10 bg-white/5"
                    >
                      {skill}
                    </span>
                  ))}
            ...
            <div className="space-y-6">
              ...
              {resumeData.patents.map((patent, i) => (
                <a
                  ...
                  className="block p-6 transition-all border rounded-xl glass-card hover:border-cyan-400/50 hover:bg-white/10 group"
                >
                  <div className="flex items-start justify-between gap-4">
                    <h4 className="font-bold text-white transition-colors group-hover:text-cyan-400">
                      "{patent.title}"
                    </h4>
                  ...
                  <div key={i} className="p-5 border rounded-xl glass-card">
                    ...
                    <div className="flex justify-between mt-2 text-sm text-slate-400">
                      ...
                      <div className="space-x-4">
                        {edu.gpa && (
                          <span className="text-cyan-400">GPA: {edu.gpa}</span>
                        )}
                        <span>{edu.dates}</span>
                      </div>
                    </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                {resumeData.awards.map((award, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-3 p-4 border rounded-lg glass-card text-slate-300 hover:bg-white/5 transition-colors"
                  >
